# lucidlint — the Rust scan core

The deterministic per-file + repo-wide finding engine behind the lucidlint
gate. `lucidlint.py` requires it (one invocation per repo; the gate fails
...
- Line/line-1 `# lucidlint: ignore <kind> <why>` and `ignore-file`
  suppressions are applied before output; a why-less suppression is itself
  a finding.

## LSP server (built in)

`lucidlint --lsp` is a stdio JSON-RPC language server. Point an
