...
| **wide-tuple → latent-class** | fail | Python | A tuple annotation with 3+ fixed elements (parameter, return, or variable) — an anonymous record: the positions carry meaning the call site cannot see. 2-tuples are pairs; tuple[X, ...] is a variadic sequence, not a record. 4+ elements fail, 3 warns. |
| **data-clump → latent-class** | fail | Python | >=3 module functions sharing the same parameter pair — the pair travels together, so it is a data clump. Two directions: give the clump its own class named with a domain noun (a subset of a larger record may legitimately be its own value), or — when an existing class's fields already account for the clump (exact names, or derived properties like `scale.unit`) — make the functions methods of that class. |
